...
export default function PricingCard({
  ...
  return (
    <div
      className={
        'relative flex w-full max-w-sm flex-col rounded-3xl border ' +
        (isPopular
          ? 'border-cyan-500/50 bg-white/[0.06]'
          : 'border-white/12 bg-white/[0.03]') +
        ' p-8 shadow-2xl transition duration-300 hover:border-white/30'
      }
    >
      {isPopular && (
      ...
      <button
        onClick={onSubscribe}
        className={
          'mt-auto w-full rounded-xl px-4 py-3 text-sm font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 ' +
          (isPopular
