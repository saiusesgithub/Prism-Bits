      ...
      <Container className="relative pb-24">
        <section className="max-w-3xl">
          ...
          <p className="mt-5 text-base leading-7 text-muted sm:text-lg">
            This page will collect real examples, remixes, and product screens
            that use Prism Bits components in production.
          </p>
        </section>
        ...
        <section className="mt-12 grid gap-4 md:grid-cols-3">
          {placeholders.map((name) => (
            <article
              key={name}
              className="group flex min-h-80 flex-col justify-between overflow-hidden rounded-[1.5rem] border border-white/10 bg-card/70 p-5 shadow-2xl shadow-purple-950/10 backdrop-blur-2xl transition hover:-translate-y-1 hover:border-cyan-200/25"
            >
              <div>
                ...
                <p className="mt-3 text-sm leading-6 text-muted">
                  A reserved showcase slot for a future community submission.
                </p>
              </div>
              <Button
                href="/components"
                variant="ghost"
                className="mt-8 justify-start px-0"
              >
                Browse bits
