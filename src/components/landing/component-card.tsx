      ...
      <div className="p-5">
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {component.tags.map((tag) => (
            <span
              key={tag}
              className="rounded-full bg-white/[0.055] px-3 py-1 text-xs text-white/55"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="mt-6 grid grid-cols-2 gap-3">
          ...
          <Button
            href={`https://github.com/saiusesgithub/Prism-Bits/tree/main/registry/${component.framework}/${component.category}/${component.slug}`}
            variant="ghost"
            className="h-11 rounded-xl border border-white/10 bg-black/20 text-sm"
          >
