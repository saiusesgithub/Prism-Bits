...
const previewMapSource = existsSync(PREVIEW_MAP_PATH)
  ? readFileSync(PREVIEW_MAP_PATH, 'utf8')
  : null;
...
function collectCategorySlugs() {
  const match = registrySource.match(
    /export const componentCategories[\s\S]*?=\s*\[([\s\S]*?)\];/
  );
  if (!match) return [];

  return [...match[1].matchAll(/slug:\s*"([^"]+)"/g)].map(
    (slugMatch) => slugMatch[1]
  );
}

const categorySlugs = new Set(collectCategorySlugs());
const metaPaths = await fg(`${REGISTRY_ROOT}/**/meta.json`, {
  onlyFiles: true,
});
...
for (const metaPath of metaPaths) {
  ...
  const relativePath = relative(REGISTRY_ROOT, componentDir)
    .split(sep)
    .join('/');
  ...
  if (segments.length !== 3) {
    fail(
      componentDir,
      `meta.json must live at ${REGISTRY_ROOT}/<framework>/<category>/<slug>/ (found depth ${segments.length}: "${relativePath}")`
    );
    continue;
  }
  ...
  if (!allowedFrameworks.has(folderFramework)) {
    fail(
      componentDir,
      `framework directory "${folderFramework}" is not supported (expected one of: ${[...allowedFrameworks].join(', ')})`
    );
  }
  if (!categorySlugs.has(folderCategory)) {
    fail(
      componentDir,
      `category directory "${folderCategory}" is not a known category from src/data/components-registry.ts`
    );
  }
  if (!slugPattern.test(folderSlug)) {
    fail(
      componentDir,
      `component directory "${folderSlug}" must be lowercase kebab-case (e.g. "retro-shadow-button")`
    );
  }
  ...
  for (const field of [
    ...
    if (meta[field] === undefined)
      fail(componentDir, `meta.json is missing required field "${field}"`);
  }
  ...
  if (typeof meta.slug === 'string' && !slugPattern.test(meta.slug)) {
    fail(
      componentDir,
      `"slug" must be lowercase kebab-case, got "${meta.slug}"`
    );
  }
  if (typeof meta.slug === 'string' && meta.slug !== folderSlug) {
    fail(
      componentDir,
      `folder name "${folderSlug}" does not match meta.json slug "${meta.slug}"`
    );
  }
  if (typeof meta.category === 'string' && !categorySlugs.has(meta.category)) {
    fail(
      componentDir,
      `"category" references unknown category "${meta.category}"`
    );
  }
  if (
    typeof meta.category === 'string' &&
    categorySlugs.has(folderCategory) &&
    meta.category !== folderCategory
  ) {
    fail(
      componentDir,
      `category directory "${folderCategory}" does not match meta.json category "${meta.category}"`
    );
  }
  if (
    typeof meta.framework === 'string' &&
    !allowedFrameworks.has(meta.framework)
  ) {
    fail(
      componentDir,
      `"framework" must be one of: ${[...allowedFrameworks].join(', ')}, got "${meta.framework}"`
    );
  }
  if (
    typeof meta.framework === 'string' &&
    allowedFrameworks.has(folderFramework) &&
    meta.framework !== folderFramework
  ) {
    fail(
      componentDir,
      `framework directory "${folderFramework}" does not match meta.json framework "${meta.framework}"`
    );
  }
  ...
  if (meta.status !== undefined && !allowedStatuses.has(meta.status)) {
    fail(
      componentDir,
      `"status" must be one of: ${[...allowedStatuses].join(', ')}, got "${meta.status}"`
    );
  }
  if (
    meta.difficulty !== undefined &&
    !allowedDifficulties.has(meta.difficulty)
  ) {
    fail(
      componentDir,
      `"difficulty" must be one of: ${[...allowedDifficulties].join(', ')} (lowercase), got "${meta.difficulty}"`
    );
  }
  ...
  if (
    ...
    (!Array.isArray(meta.tags) ||
      !meta.tags.every((tag) => typeof tag === 'string' && tag.trim()))
  ) {
  ...
  if (
    ...
    (!Array.isArray(meta.dependencies) ||
      !meta.dependencies.every((dep) => typeof dep === 'string' && dep.trim()))
  ) {
    fail(
      componentDir,
      `"dependencies" must be an array of non-empty strings when supplied`
    );
  }
  if (meta.author !== undefined) {
    if (
      ...
      !meta.author.name.trim()
    ) {
      fail(
        componentDir,
        `"author.name" is required and must be a non-empty string`
      );
    }
    if (
      meta.author?.github !== undefined &&
      (typeof meta.author.github !== 'string' ||
        !githubUsernamePattern.test(meta.author.github))
    ) {
      fail(
        componentDir,
        `"author.github" must be a valid GitHub username when supplied, got "${meta.author?.github}"`
      );
    }
  }
  ...
  if (typeof meta.category === 'string' && typeof meta.slug === 'string') {
    ...
    if (seenRoutes.has(routeKey)) {
      fail(
        componentDir,
        `duplicate route "${routeKey}" — already declared by ${seenRoutes.get(routeKey)}`
      );
    } else {
  ...
  if (
    ...
    typeof meta.framework === 'string' &&
    requiredFilesByFramework[meta.framework]
  ) {
    for (const fileName of requiredFilesByFramework[meta.framework]) {
      if (!existsSync(join(componentDir, fileName))) {
        fail(
          componentDir,
          `status is "available" but required ${meta.framework} file "${fileName}" is missing`
        );
      }
    }
  }
  ...
  if (meta.framework === 'react') {
    ...
    if (hasPreviewTsx && !hasComponentTsx) {
      fail(
        componentDir,
        `preview.tsx exists but component.tsx is missing — preview.tsx is only a demo wrapper for component.tsx`
      );
    }
    if (hasComponentTsx && !hasDefaultExport(componentTsxPath)) {
      fail(
        componentDir,
        `component.tsx must default-export the reusable React component`
      );
    }
    if (hasPreviewTsx && !hasDefaultExport(previewTsxPath)) {
      fail(
        componentDir,
        `preview.tsx must default-export a zero-prop preview component`
      );
    }
    ...
    if (meta.status === 'available') {
      if (!previewMapSource) {
        fail(
          componentDir,
          `no generated preview map found at ${PREVIEW_MAP_PATH} — run "npm run generate:react-previews"`
        );
      } else if (
        !previewMapSource.includes(`"${folderCategory}/${folderSlug}":`)
      ) {
        fail(
          componentDir,
          `status is "available" but "${folderCategory}/${folderSlug}" has no entry in ${PREVIEW_MAP_PATH} — run "npm run generate:react-previews"`
        );
      }
    }
  }
...
console.log(
  `Registry validation passed: ${categorySlugs.size} categories, ${metaPaths.length} components, 0 errors.`
);
